...
namespace ElibraryManagement
{
    public partial class adminbookinventory : System.Web.UI.Page
    {
        ...
        protected void Button2_Click(object sender, EventArgs e)
        {
            ...
            {
                Response.Write("<script>alert('Book ID already exsists.Try some other book ID');</script>");
            }
            else
            {
                addNewBook();   
        ...
        void deleteBookById()
        {
            if (checkIfBookExsist())
            {
                try
                {
                    ...
                    SqlCommand cmd = new SqlCommand("DELETE from book_master_tbl where book_id='" + TextBox7.Text.Trim() + "'",
                        con);
        ...
        void updateBookById()
        {
            if (checkIfBookExsist())
            {
                try
                {
                    ...
                    SqlCommand cmd = new SqlCommand("UPDATE book_master_tbl SET book_name=@book_name, genre=@genre, author_name=@author_name, publisher_name=@publisher_name, publish_date=@publish_date, language=@language," +
                        " edition=@edition, book_cost=@book_cost, no_of_pages=@no_of_pages, book_description=@book_description, actual_stock=@actual_stock, current_stock=@current_stock, book_img_link=@book_img_link where book_id='" + TextBox7.Text.Trim() + "'", con);
